	...
	$(window).load(function(){
		// 侧边栏自适应
	     ...
	     $('nav dl dt ul.menu-product li').each(function(){
	         ...
	         if((TopLi+HegihtMenu)>(HeightParents+TopParents) && HegihtMenu < HeightParents){
	            $(this).find('.menu-child').css({"top":TopMenu+'px'});
	         }else if(HegihtMenu >= HeightParents){
	         	var TopThisLi = $(this).offset().top;
	         	$(this).find('.menu-child').css({"top":(-TopThisLi+TopParents)+'px'});
	         }
	    });
	});
	</script>
