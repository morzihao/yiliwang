...
<body>
	...
	<!-- 内容 -->
     ...
     <div  class="member_left_menu">
        <dl>
           ...
           <dd><a href="javascript:;" class="selected">店铺管理</a>
              <ul>
        ...
        <div class="member_right">
            ...
            <form action="#">
               <dl class="add-sort">
                 <dt><a href="javascript:;" class="add-fs">+&nbsp;&nbsp;添加分类</a></dt>
                 <dd>
                   <!-- 全选操作 -->
                   <div class="allCheck-operation">
                     <input type="checkbox" class="all-check checkbox">
                     <label for="">全选</label>
                     <input type="button" class="batch-delete" value="批量删除">
                   </div>
                   <!-- 全选操作 结束 -->
                   <!-- 分类列表标题 -->
                   <ul class="title-sort">
                     <li style="width:440px;">分类名称</li>
                     <li style="width:235px;">移动</li>
                     <li style="width:108px;">创建时间</li>
                     <li style="width:195px; text-indent: 27px;">操作</li>
                   </ul>
                   <!-- 分类列表标题 结束 -->
                   <!-- 分类列表 内容 -->
                    <div class="con-sortList">
                      <!-- 大分类 -->
                      <!-- <div class="total-sort">
                          <ul class="first-sort">
                             <li class="fs01"><input type="checkbox" class="checkbox"></li>
                             <li class="fs02"><span class="open"></span><input type="text" value="添加分类"></li>
                             <li class="fs03"><a href="javacript:;" class="add-cs">+&nbsp;添加子分类</a></li>
                             <li class="fs04"><input type="button" class="up-fs"><input type="button" class="down-fs"></li>
                             <li class="fs05">2014-09-20</li>
                             <li class="fs06"><a href="javascript:;" class="delete-fs">删除</a></li>
                          </ul> -->
                          <!-- 小分类 列表 -->
                          <!-- <div class="child-list">
                              <ul class="child-sort">
                                <li class="cs01"><input type="checkbox" class="checkbox"></li>
                                <li class="cs02"><input type="text" value="添加子分类"></li>
                                <li class="cs03"><input type="button" class="up-cs"><input type="button" class="down-cs"></li>
                                <li class="cs04">2014-09-20</li>
                                <li class="cs05"><a href="javascript:;" class="delete-cs">删除</a></li>
                              </ul>
                          </div> -->
                          <!-- 小分类 列表 结束-->
                      <!-- </div> -->
               ...
               <input type="submit" value="保存" class="submit-sort">
               </form>
        	</div>
                <!-- page -->
  ...
  $(function(){
    // 添加大分类
    $('.add-fs').click(function(){
       addFs();
    });
    function addFs(){
      var totalSort ='<div class="total-sort">\
                          <ul class="first-sort">\
                             <li class="fs01"><input type="checkbox" class="checkbox"></li>\
                             <li class="fs02"><span class="open"></span><input type="text" value="添加分类"></li>\
                             <li class="fs03"><a href="javacript:;" class="add-cs">+&nbsp;添加子分类</a></li>\
      ...
      $('.con-sortList').append(totalSort);
    };
    // 删除大分类
    $('.delete-fs').live({
       click:function(){
                var conf = confirm('您确定删除此分类吗？');
                if (conf) {
                   $(this).parents('.total-sort').remove();
                }
            }
    });
    // 移动大分类 上移
    $('.up-fs').live({
      click:function(){
          $(this).parents('.total-sort').prev().before($(this).parents('.total-sort'));
      }
    });
    // 移动大分类 下移
    $('.down-fs').live({
       click:function(){
          $(this).parents('.total-sort').next().after($(this).parents('.total-sort'));
        }
    });
    // 显示 隐藏 小分类
    $('.open').live({
        click:function(){
          ...
          $(this).parents('.first-sort').next('.child-list').slideToggle(200);
        }
    });
    // 添加小分类
    $('.add-cs').live({
        click:function(){
          $(this).parents('.first-sort').find('.open').addClass('open-down');
          var num = $(this).parents('.total-sort').index();
          $(this).parents('.total-sort').find('.child-list').css({'display':'block'});
          function addCs(){
              var smallSort ='<ul class="child-sort">\
                                <li class="cs01"><input type="checkbox"></li>\
                                <li class="cs02"><input type="text" value="添加子分类"></li>\
                                <li class="cs03"><input type="button" class="up-cs"><input type="button" class="down-cs"></li>\
                                <li class="cs04">2014-09-20</li>\
                                <li class="cs05"><a href="javascript:;" class="delete-cs">删除</a></li>\
                              </ul>';
               $('.child-list').eq(num).append(smallSort);
          }
             addCs();
      }
    });
    ...
    $('.delete-cs').live({
       click:function(){
                var conf = confirm('您确定删除此分类吗？');
                if (conf) {
                   $(this).parents('.child-sort').remove();
                }
            }
    });
        // 移动小分类 上移
    $('.up-cs').live({
      click:function(){
          $(this).parents('.child-sort').prev().before($(this).parents('.child-sort'));
      }
    });
    // 移动小分类 下移
    $('.down-cs').live({
       click:function(){
          $(this).parents('.child-sort').next().after($(this).parents('.child-sort'));
        }
    });
    // 全选 /全不选
